...
# A same-version test cannot show this: the standby is a physical copy of the old
# cluster, so if old and new are the same build its data matches the upgraded
# primary whether or not any upgrade happened.  Across 18 -> 20 the old on-disk
# cluster is v18 (catalog version 202506291); only a real replay of the upgrade
# WAL turns it into a working v20 cluster (catalog 202607022).
#
...
# skeleton STREAMS the upgrade window from the live primary (local
# CN derivation; NO cp) -> it replays from CN and becomes a v20 hot standby
# that matches the upgraded primary.
#
# Requires: OLDBIN (an 18.x bin dir) and NEWBIN (the 20devel wal_upgrade
# build).  Set them via env; defaults match the CI layout used in development.
set -u
...
# A physical slot on the old primary marks that a standby is expected; pg_upgrade
# migrates it and it pins the upgrade window so the new-version skeleton can
# stream it (pg_upgrade creates no dedicated slot when none exists).
...
# STREAM the window from it (the migrated physical slot pins the window so it
# survives the upgrade -- no manual WAL copy).
...
# The upgrade window carries only the system delta plus the relink manifest, NOT
# user data.  The standby target is a FRESH NEW-version ($NEWVER) initdb skeleton
# -- it must be new-version, since the streamed window is new-format.  Its
# pg_upgrade_standby_old_datadir GUC names the retained OLD-version datadir ($STBY, the
# v18 basebackup from step 1), whose user relations the manifest redo copies into
# the skeleton (relfilenumbers preserved across the major upgrade, so paths
# match).  It then STREAMS the window from the live upgraded primary via
# local CN derivation (sysid via IDENTIFY_SYSTEM).
#
# NOTE: we deliberately do NOT stamp the sysid.  The fresh skeleton keeps its own
# initdb sysid; it takes the primary's sysid from IDENTIFY_SYSTEM and derives CN locally over the
# replication connection so the walreceiver accepts the primary and recovery
# starts at CN.  This proves pg_resetwal --system-identifier is not needed:
...
# skeleton derives CN locally; only sysid comes over this replication connection.
