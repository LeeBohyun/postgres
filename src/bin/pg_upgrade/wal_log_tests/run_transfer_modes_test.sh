...
reflink_supported() {
  ...
  echo x > "$probe/a"
  cp --reflink=always "$probe/a" "$probe/b" 2>/dev/null && return 0
  return 1
}

if reflink_supported; then
  MODES="copy clone copy-file-range link swap"
else
  log "SKIP --clone/--copy-file-range: filesystem does not support reflinks ($(stat -f -c '%T' "$W" 2>/dev/null))"
  MODES="copy link swap"
fi
