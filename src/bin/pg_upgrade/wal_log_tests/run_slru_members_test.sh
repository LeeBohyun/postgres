...
# A physical slot makes --wal-upgrade retain the window in the new cluster's
# pg_wal/ (a standby is expected) so the waldump scan below can find the SLRU
# records; without one the window is recycled as ordinary WAL.
...
echo "  members RAWFILE records in upgrade WAL:"; sed 's/^/    /' "$W/members_recs.txt" | head
MAXSEG=$(grep -oiE 'pg_multixact/members/[0-9A-F]+' "$W/members_recs.txt" \
         | sed -E 's#.*/0*([0-9A-F]+)$#\1#' | sort -rn | head -1)
log "highest members segment captured in WAL: '${MAXSEG:-none}'"
if [ -z "$MAXSEG" ] || echo "$MAXSEG" | grep -qE '^0+$'; then
  echo "  FAIL: no non-zero members segment captured in the window"; FAIL=1
else
  echo "  OK: non-zero members segment $MAXSEG captured with its correct name"
fi
