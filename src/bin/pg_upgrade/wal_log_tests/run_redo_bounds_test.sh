...
#
# PG_UPGRADE_TEST_CORRUPT_RAWFILE_LEN (a test-only fault-injection hook) makes
# the RAWFILE emit path stamp data_len larger than the payload it actually
# registers, exactly the shape a corrupt/hostile record would have.  SLRU
# segments and PG_VERSION are captured as XLOG_UPGRADE_RAWFILE records, so this
# covers the bounds check that guards all of them.
#
...
# halt at the corrupt RAWFILE record with the bounds-check message rather than
# over-reading the WAL buffer or converging on truncated state.
#
...
log "2. upgrade the primary (--wal-upgrade) with an inflated RAWFILE data_len"
cd "$W"
PG_UPGRADE_TEST_CORRUPT_RAWFILE_LEN=1 "$BIN/pg_upgrade" -b "$BIN" -B "$BIN" -d "$OLD" -D "$NEW" -U postgres \
...
primary_slot_name='redo_bounds_slot'
pg_upgrade_standby_old_datadir='$OLD'
CONF
...
for i in $(seq 1 30); do
    grep -qiE "rawfile record overruns the record" "$W/standby.log" && break
    sleep 1
done
if grep -qiE "rawfile record overruns the record" "$W/standby.log"; then
    log "  redo REJECTED the malformed RAWFILE record (good):"
    grep -iE "rawfile record overruns" "$W/standby.log" | tail -1
else
    echo "  FAIL: standby did not halt with the bounds-check message; log tail:"
    tail -15 "$W/standby.log"; FAIL=1
fi
# The standby must NOT have converged to a queryable state on a corrupt record.
...
echo "========================================================================"
[ "$FAIL" = 0 ] && log "PASS: redo rejected the malformed RAWFILE record during standby replay" \
