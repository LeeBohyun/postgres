...
if "$BIN/pg_upgrade" --wal-upgrade-signal-handoff -b "$BIN" -d "$W/old" -U postgres >"$W/d2.log" 2>&1; then
    fail "D2: signal-handoff succeeded against a stopped primary (should fail)"
fi
...
if "$BIN/pg_upgrade" --wal-upgrade-signal-handoff -b "$BIN" -d "$W/old3" -U postgres >"$W/d3.log" 2>&1; then
    fail "D3: signal-handoff succeeded against a never-started primary (should fail)"
fi
grep -qiE "could not shut down the old primary|is server running|PID file.*does not exist|could not connect" "$W/d3.log" || { cat "$W/d3.log"; fail "D3: wrong/absent failure message"; }
[ -f "$W/old3/pg_upgrade_handoff.pending" ] && fail "D3: handoff sentinel left behind"
[ -f "$W/old3/global/pg_control" ] || fail "D3: cluster's pg_control disturbed"
log "PASS D3 (clean failure, cluster undamaged)"
