...
# var and can never emit an incomplete window), so this test only runs on a
# cassert build; on a non-assert build it SKIPS.
set -u
...
if ! grep -q "^#define USE_ASSERT_CHECKING 1" "$("$BIN/pg_config" --includedir)/pg_config.h" 2>/dev/null; then
    echo "=== SKIP: crash-path fault injection requires a cassert build ==="
    exit 0
fi
...
log "partial cluster: upgrade_finalized flag unset (nothing to adopt)"
# The pg_control upgrade_finalized flag must be UNSET for a crash-truncated window.
if "$BIN/pg_controldata" -D "$NEW" 2>/dev/null | grep -q "wal-upgrade window finalized: *yes"; then
  echo "FAIL: upgrade_finalized set on a partial (no-COMPLETE) cluster"; FAIL=1
else
  echo "upgrade_finalized unset (good)"
fi
