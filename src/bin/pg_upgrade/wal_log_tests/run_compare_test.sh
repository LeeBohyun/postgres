...
# NOT authoritative cluster content (report-only):
...
for rel in common:
    ...
    # upgrade and a WAL replay.  Report but do not fail (report-only).
    if is_vm_fsm(rel):
...
# divergence; empty-only files are expected and benign (report-only).
