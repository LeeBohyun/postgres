...
# Then "pg_upgrade --wal-log-delete-old" on the STANDBY removes the standby's old cluster,
...
CONF
"$BIN/pg_upgrade" -B "$BIN" -D "$SNEW" --wal-log-prepare-standby >"$W/prep.log" 2>&1 \
...
# local --wal-log-delete-old gate honoring the marker.
"$BIN/initdb" -D "$SOLD" -U postgres -N >/dev/null 2>&1
mv "$SOLD/global/pg_control" "$SOLD/global/pg_control.old"   # superseded stamp

log "3. --wal-log-delete-old on the PRIMARY: deletes primary old dir + emits delete-authorize"
"$BIN/pg_upgrade" -b "$BIN" -B "$BIN" -d "$POLD" -D "$PNEW" -U postgres --wal-log-delete-old >"$W/pdel.log" 2>&1
...
log "5. --wal-log-delete-old on the STANDBY: removes its (stand-in) old dir; marker = fleet authorization"
"$BIN/pg_upgrade" -B "$BIN" -D "$SNEW" -d "$SOLD" --wal-log-delete-old >"$W/sdel.log" 2>&1
...
echo "========================================================================"
[ "$FAIL" = 0 ] && log "PASS: --wal-log-delete-old on the primary signaled the standby set-wide; standby honored it and deleted its old cluster" \
