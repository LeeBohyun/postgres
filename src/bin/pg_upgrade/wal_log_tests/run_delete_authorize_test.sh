...
# committed and live.  Running "pg_upgrade --delete-old" on the PRIMARY must:
...
# Then "pg_upgrade --delete-old" on the STANDBY removes the standby's old cluster,
...
cd "$W"
"$BIN/pg_upgrade" -b "$BIN" -B "$BIN" -d "$POLD" -D "$PNEW" -U postgres --initdb --wal-log-upgrade --copy >"$W/up.log" 2>&1
...
"$BIN/pg_upgrade" -b "$BIN" -B "$BIN" -d "$POLD" -D "$PNEW" -U postgres --commit >"$W/pcommit.log" 2>&1 \
...
log "2. bring up a STREAMING standby via --prepare-standby (no hold, no commit)"
...
CONF
"$BIN/pg_upgrade" -B "$BIN" -D "$SNEW" --prepare-standby >"$W/prep.log" 2>&1 \
...
# local --delete-old gate honoring the marker.
"$BIN/initdb" -D "$SOLD" -U postgres -N >/dev/null 2>&1
mv "$SOLD/global/pg_control" "$SOLD/global/pg_control.old"   # superseded stamp

log "3. --delete-old on the PRIMARY: deletes primary old dir + emits delete-authorize"
"$BIN/pg_upgrade" -b "$BIN" -B "$BIN" -d "$POLD" -D "$PNEW" -U postgres --delete-old >"$W/pdel.log" 2>&1
...
log "5. --delete-old on the STANDBY: removes its (stand-in) old dir; marker = fleet authorization"
"$BIN/pg_upgrade" -B "$BIN" -D "$SNEW" -d "$SOLD" --delete-old >"$W/sdel.log" 2>&1
...
echo "========================================================================"
[ "$FAIL" = 0 ] && log "PASS: --delete-old on the primary signaled the standby set-wide; standby honored it and deleted its old cluster" \
