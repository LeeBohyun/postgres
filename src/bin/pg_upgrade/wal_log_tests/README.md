# --wal-upgrade manual test harnesses

Bash harnesses that exercise `pg_upgrade --wal-upgrade` end to end: they
...
| `run_compare_test.sh` | Diffs a `--wal-upgrade`+replay cluster against a normal pg_upgrade of identical data, page by page (LSN/checksum aware) |
