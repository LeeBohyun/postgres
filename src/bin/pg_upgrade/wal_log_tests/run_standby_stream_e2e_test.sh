...
# confirm the retention slot pinning the window is present (the anchor itself is
# fetched by the standby over replication via PG_UPGRADE_WINDOW_ANCHOR, not SQL)
SLOT=$("$BIN/psql" -h "$W" -p $PP -U postgres -tAc "SELECT slot_name FROM pg_replication_slots WHERE slot_name='pg_upgrade_window'")
log "retention slot='$SLOT'"
