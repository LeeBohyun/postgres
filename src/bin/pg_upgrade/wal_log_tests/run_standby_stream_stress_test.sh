...
CONF
  ...
  ls "$OLD"/base/*/[0-9]* >/dev/null 2>&1 \
    || { echo "FAIL: $shape no user relations in the retained old datadir"; GRC=1; }
...
echo "========================================================================"
[ "$GRC" = 0 ] && log "PASS: streaming-standby stress -- all shapes streamed (no manual WAL copy) and converged to the upgraded primary" \
