...
#
{
	...
	# A finalized cluster is recognized from the durable
	# pg_upgrade_complete.done marker, which the COMPLETE redo fsync'd.  This is
	# what keeps a completed upgrade whose window tail is later torn or trimmed
	# from being misdiagnosed as a crashed mid-window upgrade (the scan may miss
	# COMPLETE, but the marker still says finalized).
	ok(-f $new->data_dir . '/pg_upgrade_complete.done',
		'happy: durable COMPLETE marker present on a finalized cluster');
...
# encodes that workflow end to end.
{
	...
	# On the backup/PITR path the window is delivered via the archive, so
	# pg_upgrade drops the retention slot once the window is archived; otherwise
	# it would pin pg_wal/ forever (max_slot_wal_keep_size defaults to -1) with
	# no standby to consume it.  Confirm no physical slot lingers.
	is($new->safe_psql('postgres',
			"SELECT count(*) FROM pg_replication_slots WHERE slot_type = 'physical'"),
		'0', 'pitr: upgrade-window retention slot dropped after archiving');
	...
	# would reject at the version gate.  Drop pg_upgrade_recovery.signal next to
	# the staged window so the new binary, seeing recovery.signal + this
	# sentinel in checkDataDir(), synthesizes a new-version pg_control before the
	# gate.  Same-version needs no synthesis and is unaffected (control file
	# already matches), but the sentinel is harmless there.
	open(my $u2, '>', $restore->data_dir . '/pg_upgrade_recovery.signal') or die $!;
	close($u2);
