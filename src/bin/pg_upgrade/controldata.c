...
/*
 * LEE: Read the full 64-bit "Database system identifier" from pg_controldata.
 *
 * The cached controldata.ident field is uint32 (it only backs a length-compat
 * check), so it truncates the identifier.  --wal-log-upgrade needs the exact
 * 64-bit value to stamp into the new cluster (see create_new_cluster_via_initdb),
 * so read it fresh here.
 */
uint64
get_control_system_identifier(ClusterInfo *cluster)
{
	char		cmd[MAXPGPATH];
	char		bufin[MAX_STRING];
	FILE	   *output;
	...
	while (fgets(bufin, sizeof(bufin), output))
	{
		if ((p = strstr(bufin, "Database system identifier:")) != NULL)
		{
			p = strchr(p, ':');
			if (p == NULL || strlen(p) <= 1)
				pg_fatal("%d: controldata retrieval problem", __LINE__);
	...
	if (!got)
		pg_fatal("could not read system identifier from pg_controldata");
