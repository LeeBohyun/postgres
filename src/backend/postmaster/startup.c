...
StartupProcessMain(const void *startup_data, size_t startup_data_len)
{
	...
	/*
	 * If pg_wal_upgrade/ exists, copy its WAL segments into pg_wal/ and let
	 * StartupXLOG() crash-recover from C0 (initdb checkpoint) through
	 * XLOG_UPGRADE_COMPLETE.  If pg_wal_upgrade/ exists but lacks COMPLETE
