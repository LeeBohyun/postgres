 ...
 *
 * The individual steps are driven from C by EmitUpgradeWalWindow() below,
 * reachable only through the binary-upgrade-gated SQL entry point
 * binary_upgrade_emit_wal_window(), so they are not exposed as reusable SQL on
 * a normal cluster.
 */
 ...
 * as WAL.  Relation files -> ordinary XLOG_FPI records (replayed by stock
 * xlog_redo through the buffer manager); pg_filenode.map -> XLOG_RELMAP_UPDATE;
 * PG_VERSION -> XLOG_UPGRADE_RAWFILE (written verbatim, as it is neither a
 * buffered relation nor a relation map).  Everything else (pg_internal.init and
 * friends) is regenerated by the server and skipped.
 */
static void
				  ...
				  const char *reldir, Oid tsoid, Oid dboid, uint32 old_major)
{
	...
	while ((de = ReadDir(dir, reldir)) != NULL)
	{
		...
		{
			/*
			 * The relation map rides the ordinary XLOG_RELMAP_UPDATE record
			 * rather than a verbatim file image: its redo derives the
			 * destination from (dbid, tsid), exactly as it does for a
			 * database created by CREATE DATABASE.  dboid is InvalidOid for
			 * the shared map in global/.
			 */
 ...
 * under base/<dboid>/ and global/, plus the relation maps and PG_VERSION, so
 ...
 *
 * Emit the whole --wal-upgrade window as WAL from one routine, so the frontend
 * drives the capture with a single gated backend call.  The order is fixed:
 *
 *   1. flush SLRU dirty pages, without a checkpoint.  CN -- the recovery
 *      anchor -- is the DB_SHUTDOWNED checkpoint pg_resetwal wrote before the
 *      burst server started, and a checkpoint here would displace it.
					 ...
					 bool skip_complete)
{
	 ...
	 * pg_upgrade before this call; see pg_upgrade.c for why it must reserve
	 * before CN.
	 *
